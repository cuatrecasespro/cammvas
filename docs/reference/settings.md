...
| Center node during arrow navigation | Always center the selected node instead of moving the viewport only when needed | Off |
...
| Navigation zoom padding | Extra space around the target node when zooming (px). 0 = tight zoom. | 200 |

{% hint style="info" %}
Settings apply to all canvases in the vault. Mindmap mode can be toggled per-canvas using the brain icon in the canvas toolbar.
{% endhint %}
